    ...
    const fetchStats = async () => {
        try {
            const [attendanceRes, gradesRes, subjectsRes, assignmentsRes] = await Promise.all([
                axios.get(`${import.meta.env.VITE_API_URL}/attendance/student/${userInfo._id}/stats`).catch(() => ({ data: { percentage: 0, present: 0, absent: 0, late: 0, total: 0 } })),
                axios.get(`${import.meta.env.VITE_API_URL}/grades/student/${userInfo._id}/stats`).catch(() => ({ data: { averageMarks: 0 } })),
                axios.get(`${import.meta.env.VITE_API_URL}/subjects`).catch(() => ({ data: [] })),
                axios.get(`${import.meta.env.VITE_API_URL}/assignments`).catch(() => ({ data: [] }))
            ]);

            // Get student's grades by subject
            const studentGrades = await axios.get(`${import.meta.env.VITE_API_URL}/grades/student/${userInfo._id}`).catch(() => ({ data: [] }));
