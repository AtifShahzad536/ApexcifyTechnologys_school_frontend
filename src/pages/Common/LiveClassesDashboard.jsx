        ...
        <div className="bg-gray-50 min-h-screen">
            ...
            {(userInfo.role === 'Teacher' || userInfo.role === 'Student') && (
                <div className="mb-10">
                    ...
                    {loading ? (
                    ...
                    ) : (userInfo.role === 'Student' && !timetable) ? (
                        <p className="text-gray-400 italic">No timetable found.</p>
                    ) : (
                        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
                            ...
                            {/* Student View (Filter for Today's Online Classes) */}
                            {userInfo.role === 'Student' && timetable?.days?.find(d => d.day === new Date().toLocaleDateString('en-US', { weekday: 'long' }))?.periods.filter(p => p.isOnline).map((period, idx) => (
                                <motion.div
                                    key={idx}
                                    variants={cardVariants}
                                    initial="hidden"
                                ...
                                >
                                    <div className="flex justify-between items-start mb-4">
                                        <div>
                                            <h3 className="font-bold text-lg text-gray-800">{period.subject?.name}</h3>
                                    ...
                                    <button
                                        onClick={() => handleJoinClass(period, false)}
                            ...
                            {userInfo.role === 'Student' && (!timetable?.days?.find(d => d.day === new Date().toLocaleDateString('en-US', { weekday: 'long' }))?.periods.some(p => p.isOnline)) && (
                                <div className="col-span-3 text-gray-400 italic">No online classes scheduled for today.</div>
                            )}
                        </div>
                    )}
                </div>
